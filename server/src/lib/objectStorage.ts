...
export class ObjectStorageService {
  ...
  async validateAndConfirmUpload(
    ...
    const allowed = ["image/", "video/"].some((prefix) =>
      contentType.startsWith(prefix)
    );
    if (!allowed || size > 100 * 1024 * 1024) {
      await objectFile.delete().catch(() => {});
      if (!allowed) {
        throw new Error(
          "Uploaded file is not an image or video. File has been deleted."
        );
      }
      throw new Error(
        "Uploaded file exceeds the 100 MB limit. File has been deleted."
      );
    }
